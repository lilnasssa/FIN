...
class Config:
    ...
    # Override под Tooken — если у него другие id моделей, чем у Xkiro.
    # Пусто => берётся общий MODEL_* выше (id совпадает у обоих провайдеров).
    MODEL_QWEN_TOOKEN: str = os.getenv("MODEL_QWEN_TOOKEN", "").strip()
    MODEL_SONNET5_TOOKEN: str = os.getenv("MODEL_SONNET5_TOOKEN", "").strip()
    MODEL_OPUS5_TOOKEN: str = os.getenv("MODEL_OPUS5_TOOKEN", "").strip()
    MODEL_GPT55_TOOKEN: str = os.getenv("MODEL_GPT55_TOOKEN", "").strip()
    MODEL_LUNA_TOOKEN: str = os.getenv("MODEL_LUNA_TOOKEN", "").strip()
    MODEL_TERRA_TOOKEN: str = os.getenv("MODEL_TERRA_TOOKEN", "").strip()
    MODEL_SOL_TOOKEN: str = os.getenv("MODEL_SOL_TOOKEN", "").strip()
...
MODEL_CATALOG: dict[str, dict[str, Any]] = {
    "qwen": {
        ...
        "api_by_provider": {"tooken": Config.MODEL_QWEN_TOOKEN} if Config.MODEL_QWEN_TOOKEN else {},
        "tier": TIER_BASE,
        "price": "₽",
    ...
    "sonnet5": {
        ...
        "api_by_provider": {"tooken": Config.MODEL_SONNET5_TOOKEN} if Config.MODEL_SONNET5_TOOKEN else {},
        "tier": TIER_CHEAP,
        "price": "₽₽",
    ...
    "gpt55": {
        ...
        "api_by_provider": {"tooken": Config.MODEL_GPT55_TOOKEN} if Config.MODEL_GPT55_TOOKEN else {},
        "tier": TIER_CHEAP,
        "price": "₽₽",
    ...
    "opus5": {
        ...
        "api_by_provider": {"tooken": Config.MODEL_OPUS5_TOOKEN} if Config.MODEL_OPUS5_TOOKEN else {},
        "tier": TIER_PREMIUM,
        "price": "₽₽₽₽",
    ...
    "luna": {
        ...
        "api_by_provider": {"tooken": Config.MODEL_LUNA_TOOKEN} if Config.MODEL_LUNA_TOOKEN else {},
        "tier": TIER_PREMIUM,
        "price": "₽₽₽₽",
    ...
    "terra": {
        ...
        "api_by_provider": {"tooken": Config.MODEL_TERRA_TOOKEN} if Config.MODEL_TERRA_TOOKEN else {},
        "tier": TIER_EXTREME,
        "price": "₽₽₽₽₽",
    ...
    "sol": {
        ...
        "api_by_provider": {"tooken": Config.MODEL_SOL_TOOKEN} if Config.MODEL_SOL_TOOKEN else {},
        "tier": TIER_EXTREME,
        "price": "₽₽₽₽₽",
...
class LLMService:
    ...
    def _post(self, provider: dict, payload: dict) -> dict:
        ...
        try:
            ...
            # DNS/сеть недоступны — почти всегда неверный BASE_URL или
            # хостинг блокирует домен, а не ошибка кода.
            raise ProviderError(
                f"{provider['title']}: домен '{provider['base_url']}' недоступен "
                f"(DNS/сеть) — проверьте *_BASE_URL и доступ хостинга к нему: {exc.reason}"
            ) from exc
    ...
    def _call_model(
        ...
        for provider in self.providers:
            # У разных провайдеров разные ID моделей — берём override для
            # конкретного провайдера, если задан, иначе общий "api".
            model_id = (spec.get("api_by_provider") or {}).get(
                provider["name"], spec["api"]
            )
            payload = dict(base_payload, model=model_id)
            try:
    ...
    def _run(
        ...
        if not used:
            uniq = list(dict.fromkeys(problems))  # без повторов, сохраняя порядок
            raise RuntimeError(
                "Ни одна модель не ответила. " + "; ".join(uniq[-4:])
            )
...
def text_value(value: Any) -> str:
    """Приводит ответ обработчика к безопасному тексту Telegram."""
    if value is None:
        return ""
    if isinstance(value, str):
        return value.strip()
    if isinstance(value, dict):
        for key in ("text", "answer", "message", "content", "summary"):
            if key in value:
...
def chunks(text: Any, size: int = 3800) -> list[str]:
    normalized = text_value(text)
    return [normalized[i : i + size] for i in range(0, len(normalized), size)] or ["—"]
...
def kb_main() -> InlineKeyboardMarkup:
    ...
    rows.append(
        [
            InlineKeyboardButton("🎨 Генерация", callback_data="gen:menu"),
    ...
    if write:
        rows.append(
            [
                InlineKeyboardButton("📥 Импорт", callback_data="excel:wait"),
                InlineKeyboardButton("📤 Экспорт", callback_data="export:menu"),
            ]
        )
        rows.append(
            [
                InlineKeyboardButton("👤 Новый артист", callback_data="artist:add"),
                InlineKeyboardButton("↩️ Отменить операцию", callback_data="tx:undo"),
            ]
        )
    else:
        rows.append(
...
def kb_access(role: str) -> InlineKeyboardMarkup:
    ...
    if role == ROLE_OWNER:
        rows.append(
            [
                InlineKeyboardButton(
                    "🎫 Код наблюдателя", callback_data="access:invite:viewer"
                ),
                InlineKeyboardButton(
                    "🎫 Код админа", callback_data="access:invite:admin"
                ),
            ]
        )
    else:
...
async def send(
    update: Update, text: Any, keyboard: InlineKeyboardMarkup | None = None
) -> None:
    parts = chunks(text)
...
def kb_gen() -> InlineKeyboardMarkup:
    """Вкладка «Генерация»: обе кнопки без расхода токенов."""
    return InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton("📄 Документ/отчёт", callback_data="gen:doc"),
                InlineKeyboardButton("🖼 Фотоотчёт", callback_data="gen:photo"),
            ],
            [InlineKeyboardButton("⬅️ Главное меню", callback_data="nav:main")],
        ]
    )
    ...
    if not state:
        ...
        if not can_write(access["role"]):
            ...
            try:
                ...
                await show_main_menu(
                    update,
                    "⚠️ ИИ временно недоступен. Проверьте активную модель и попробуйте ещё раз.",
                )
                return
    ...
    try:
        if state == "artist:name":
            ...
            try:
                ...
                log.warning("AI detailed ask failed: %s", exc)
                await show_main_menu(
                    update,
                    "⚠️ ИИ временно недоступен. Проверьте активную модель и попробуйте ещё раз.",
                )
