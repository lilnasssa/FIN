...
class Config:
    ...
    # Ключ Anthropic (через прокси SeekAI).
    GEMINI_API_KEY: str = (
        os.getenv("ANTHROPIC_API_KEY", "").strip()
        or os.getenv("GEMINI_API_KEY", "").strip()
    )
    # Имя модели Claude, например claude-sonnet-4-6 или claude-haiku-4-5.
    GEMINI_MODEL: str = os.getenv("GEMINI_MODEL", "claude-sonnet-4-6").strip() or "claude-sonnet-4-6"
...
class GeminiService:
    """Обёртка над Anthropic Claude (через SeekAI прокси). Ключ — из ANTHROPIC_API_KEY."""
    ...
    def __init__(self, api_key: str, model: str, max_tokens: int) -> None:
        ...
        self.model = (
            "claude-sonnet-4-6" if requested.lower() == "auto" else requested
        )
        self.max_tokens = max_tokens
        ACTIVE_MODEL[""] = self.model

    # Оставлено для совместимости с точкой вызова в main(): у Anthropic
    # не нужно спрашивать список моделей, модель уже фиксирована.
    ...
    def _complete(
        ...
        full_prompt = prompt
        if json_mode:
            full_prompt += "\n\nОтветь СТРОГО валидным JSON, без пояснений и markdown-обрамления."
        try:
            ...
            if status == 429:
                raise RuntimeError(
                    "Лимит запросов исчерпан. Подождите минуту и повторите."
                ) from exc
            if status in (401, 403):
                raise RuntimeError(
                    "Ключ ANTHROPIC_API_KEY неверный или без доступа к API."
                ) from exc
            if status:
                raise RuntimeError(f"AI недоступен (код {status}).") from exc
            raise RuntimeError(f"AI недоступен: {exc}") from exc
        ...
        if not text:
            raise RuntimeError(
                "AI вернул пустой ответ (возможно, сработали фильтры безопасности)."
            )
        return text
...
async def show_main_menu(update: Update, prefix: str = "") -> None:
    ...
    model = ACTIVE_MODEL.get("") or Config.GEMINI_MODEL
...
def main() -> None:
    ...
    log.info(
        "Bot started | build=%s | label=%s | model=%s | backend=%s",
        BUILD,
        Config.LABEL_NAME,
