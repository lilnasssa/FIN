...
class Config:
    ...
    # Имя модели OpenAI, например gpt-4o-mini или gpt-4o.
    GEMINI_MODEL: str = os.getenv("GEMINI_MODEL", "anthropic/claude-sonnet-5").strip() or "anthropic/claude-sonnet-5"
...
class GeminiService:
    """Обёртка над xkiro.com (прокси-шлюз, формат vendor/model). Ключ — из OPENAI_API_KEY."""
    ...
    def __init__(self, api_key: str, model: str, max_tokens: int) -> None:
        ...
        self.model = (
            "anthropic/claude-sonnet-5" if requested.lower() == "auto" else requested
        )
        self.max_tokens = max_tokens
        ACTIVE_MODEL[""] = self.model

    # Оставлено для совместимости с точкой вызова в main(): у прокси
    ...
    def _complete(
        ...
        try:
            ...
            if status in (401, 403):
                raise RuntimeError(
                    "Ключ OPENAI_API_KEY неверный или без доступа к API."
                ) from exc
            if status:
